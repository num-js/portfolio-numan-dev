...
- **Site footer:** glass panel with brand blurb, hash anchor nav, skill highlights, and a visual-only newsletter row (Subscribe disabled — backend not wired yet). Social links use `react-icons/si` in glass circle buttons. `FooterBrand` renders the monospace `< N_Ah / >` mark in Geist Mono. Light GSAP column stagger on scroll (`data-footer-col`); no ambient loops. Mounted in `layout.tsx` after page content.
